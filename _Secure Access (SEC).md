...
conf t
 ...
 int g3
  ...
  exit
 int g4
  ip add 10.72.1.11 255.255.255.0
  no shut
  exit
 !
...
__Make Sure EDGE-72 is Configured__
...
conf t
 ...
 int gi 0/0/1
  no shut
  ip add 200.0.0.72 255.255.255.0
  desc OUTSIDE
 int loopback 0
  ip add 72.0.0.1 255.255.255.255
  desc VIRTUALIP
  exit
...
!@ospf routing edge
 router ospf 1
  ...
  network 72.0.0.1 0.0.0.0 area 0
 int gi 0/0/0
...
conf t
 ...
 ip routing
 router ospf 1
  router-id 10.72.72.4
  network 10.72.0.0 0.0.255.255 area 0
 int gi 0/1
  ip ospf network point-to-point
  end
~~~
...
!@UTM-PH
conf t
 ip route 10.0.0.0 255.0.0.0 10.72.1.4
 ip route 200.0.0.0 255.255.255.0 10.72.1.4
 end
~~~
...
### Task 1: Prevent traffic only from VLAN 100 (CUCM) to reach UTM-PH (10.72.1.11)
~~~
!@UTM-PH
config t
...
### Task 2: Prevent traffic coming from the PC (10.72.1.10) and the EDGE Router from reaching UTM-PH (10.72.1.11)
~~~
!@UTM-PH
config t
...
### Task 3: Allow the PC (10.72.1.10) to access HTTP, SSH, and ICMP of UTM-PH (10.72.1.11)
...
!@UTM-PH
config t
 ip host www.bet11.com 10.72.1.11
 service finger
 ...
 telephony-service
  ...
  max-ephones 5
  max-dn 20
  ip source-address 10.72.1.11 port 2000
  exit
 voice service voip
 ...
 voice register global
  mode cme
  source-address 10.72.1.11 port 5060
  max-dn 12
  max-pool 12
...
- Allow Pings destined for your PC (10.72.1.10)  
...
config t
 ip access-list extended FWP5
  permit  icmp  any  host 10.72.1.10             log 
...
| SERVER      | INSIDE PORT | OUTSIDE IP     | OUTSIDE PORT |
| ---         | ---         | ---            | ---          |
| CoreTAAS    | 23          | 200.0.0.72 | 2023         |
| CoreBABA    | 23          | 200.0.0.72 | 4023         |
| CUCM        | 23          | 200.0.0.72 | 8020         |
...
!@EDGE-72
conf t
 ip nat inside source static tcp  10.72.1.11  6969  200.0.0.72  3306
 end
~~~
